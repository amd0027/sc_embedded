...
// Deprecated default constructor. Replaced with parameterized to avoid conflicting driver installation commands
/*
...
// Input of 0 performs one-time setup with any other values avoiding conflict
...
void SCDriverI2C::startAQSApplication()
{
	// Create temporary queue
	i2c_cmd_handle_t temp = i2c_cmd_link_create();
	if(i2c_master_start(temp) != ESP_OK)	// sends START bit and checks for error
	...
	// Address slave in write mode
	if (i2c_master_write_byte(temp, (slave_addr << 1) | I2C_MASTER_WRITE, I2C_MASTER_ACK) != ESP_OK)
		printf("Error entering read mode (in sending control bits)!\n");

	// Declare to the slave which of its registers will be written to
	if (i2c_master_write_byte(temp, 0xF4, I2C_MASTER_ACK) != ESP_OK)	// FIFO CTRL REG == 0x2E
		printf("Error entering read mode (in sending control bits)!\n");

	// write the value to the slave's register - THIS STEP IS SKIPPED TO ONLY SEND WRITE SIGNAL
	//if (i2c_master_write_byte(temp, value, I2C_MASTER_ACK) != ESP_OK)	// write to FIFO CTRL REG to put into continuous mode.
	//	printf("Error entering read mode (in sending control bits)!\n");
